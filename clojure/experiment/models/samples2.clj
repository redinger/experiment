...
(defn- chunk-update-fn
  ...
  (fn [[base-date samples]]
    (mongo/update!
     ...
     {:user (:_id u)
      :inst (:_id i)
      :start (dt/as-date base-date)}
     {:$pushAll {:samples (map as-chunk-sample samples)}
      :$set {:updated (dt/as-date (dt/now))}
      :$inc {:stats.count (count samples)
